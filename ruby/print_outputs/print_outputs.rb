...
def print_averages_errors(simulator, outputs_info, variable_info_array, get_outputs_of_run)
  simulator.parameter_sets.where(outputs_info["constants"].reduce({}) {|result, (key, value)| result["v.#{key}"] = value; return result}).each do |parameter_set|
    variables = variable_info_array.map {|variable| parameter_set.v[variable["name"]]}
    outputs_array = parameter_set.runs.where(status: :finished).map get_outputs_of_run
...
if ARGV.length == 2 then
  ...
  print_averages_errors(simulator, outputs_info, variable_info_array, {|run| output_json = JSON.load(File.open(run.dir.join("_output.json"))); output_info_array.map {|output_info| output_json[output_info["name"]]}})
else
  ...
  if analyzer[:type] == :on_run then
    ...
    print_averages_errors(simulator, outputs_info, variable_info_array, {|run| output_json = JSON.load(File.open(run.analyses.where(analyzer: analyzer, status: :finished).max_by {|analysis| analysis.created_at}.dir.join("_output.json"))); output_info_array.map {|output_info| output_json[output_info["name"]]}})
